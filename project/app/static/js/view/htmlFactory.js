...
function columnBuilder(column) {
    ...
    return `<div class="col-sm-3" data-column-id="${column.id}">
                <div class="card">
                    <div class="row card-header"> 
                        <div class="col-sm-10">
                            <h5 id="board-header" data-board-id="${column.id}">${column.title}</h5>
                        </div>
                        <div class="col-sm-2 div-button" data-column-id="${column.id}">
...
function addColumnBuilder(column){
    console.log('działa')
    return`
    <div class="input-group mb-3">
        input type="text" class="form-control add-card-imput" placeholder="Add new task" aria-label="Recipient's username" aria-describedby="basic-addon2" data-column-id="${column.id}">
            <div class="input-group-append">
                <button class="btn btn-outline-secondary add-card-button" type="button" data-column-id="${column.id}">B
